...
void Fleet::addPlane(
	...
	Fleet::utility.writeFile(Fleet::storagePlanes, Fleet::newPlanes,"../utility/data/plane.txtFleet");
}
void Fleet::addPassenger(int planeNumber, int passengerId, string seat){
	...
	for (int i = 0; i < Fleet::storagePlanes.size(); i++)
	{
		duplicatePassenger = find(
			...
			Fleet::storagePlanes[i].getPassengerIds().end(), passengerId 
	...
	for (int i = 0; i < Fleet::newPlanes.size(); i++)
	{
		duplicatePassenger = find(
			Fleet::newPlanes[i].getPassengerIds().begin(), 
			Fleet::newPlanes[i].getPassengerIds().end(), passengerId 
